...
def open_ingredient_search():
    clear_right_frame()
    Label(right_frame, text="Choose an Ingredient:").grid(row=0, column=0, padx=30, pady=10)
...
def open_effect_search():
    clear_right_frame()
    Label(right_frame, text="Enter an Effect:").grid(row=0, column=0, padx=48, pady=10)
...
Button(left_frame, text="Music On/Off", command=toggle_music ,width=12,height=2,cursor="hand2").grid(row=3, column=0, pady=20)
